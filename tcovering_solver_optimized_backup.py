...
class TCoveringSolver:
    ...
    def _fast_local_search(self, solution: list[int]) -> list[int]:
        ...
        # Build j-subset coverage counts
        j_covered_count = np.zeros(self.num_targets, dtype=np.int32)
        ...
        while improved and passes < max_passes and not self._cancel():
            ...
            for i in indices:
                ...
                # Check if removing this candidate breaks coverage
                removed_s = self._cand_covers_s[removed_idx]
                
                # Quick check: would any j-subset fall below threshold?
                can_remove = True
                for s_mask in removed_s:
                    ...
                    if s_mask in self._s_to_j:
                        for j_idx, _ in self._s_to_j[s_mask]:
                            # Check if this s-mask is the only one from removed candidate
                            # that contributes to this j-subset
                            other_count = 0
                            for other_s in self._s_subsets_per_j[int(self.target_masks[j_idx])]:
                                if other_s != s_mask and other_s in covered_s_masks:
                                    # Check if this s-mask is covered by other candidates
                                    is_covered_by_others = False
                                    for j, other_mask in enumerate(solution):
                                        if j == i:
                                            continue
                                        other_idx = self._cand_index_map.get(int(other_mask))
                                        if other_idx is not None and other_s in self._cand_covers_s[other_idx]:
                                            is_covered_by_others = True
                                            break
                                    if is_covered_by_others:
                                        other_count += 1
                            
                            # If removing would drop below threshold, can't remove
                            if j_covered_count[j_idx] - 1 < self.t and other_count < self.t:
                                can_remove = False
                                break
                ...
                if can_remove:
                    # Verify by full check (fast path failed, need accurate check)
    ...
    def _incremental_verify(self, masks: list[int], current_covered: set, removed_s: set) -> bool:
        """Fast incremental verification."""
        if not masks:
            return self.num_targets == 0
        
        # Build new covered set
