...
DEFAULT_OUTPUT_JSON = RESULTS_DIR / "n_eq_19_isolated_pipeline.json"
DEFAULT_OUTPUT_MD = RESULTS_DIR / "n_eq_19_isolated_pipeline.md"
DEFAULT_SPLIT_MD = RESULTS_DIR / "n_eq_19_isolated_pipeline_split.md"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run n=19 isolated pipeline with dedicated solver module."
    )
    ...
    parser.add_argument("--n-min", type=int, default=19)
    parser.add_argument("--n-max-exclusive", type=int, default=20)
    ...
    parser.add_argument("--solver-module", default="solver_n19_isolated")
...
def _build_cmd(args: argparse.Namespace) -> list[str]:
    return [
        ...
        "--split-md",
        str(args.split_md),
        *([] if not args.ck_skip_gpu_probe else ["--ck-skip-gpu-probe"]),
        *([] if not args.resume else ["--resume"]),
    ]


def main() -> int:
    args = _parse_args()
    if int(args.n_min) != 19 or int(args.n_max_exclusive) != 20:
        raise ValueError("n=19 独立 pipeline 只允许范围 [19, 20)。")
    cmd = _build_cmd(args)
    print("Running n=19 isolated pipeline:")
    print(" ".join(cmd))
    if args.dry_run:
        return 0
