...
def recursive_covering_indices(
    ...
    first_blocks, _ = solve_n_le_15_internal(
        first_problem, tools.make_oracle(first_problem), rng, split_deadline, tools
    )
    if time.monotonic() >= deadline:
        return tuple()
    second_blocks, _ = solve_n_le_15_internal(
...
def partial_cyclic_orbit_repair_with_seed(
    ...
    search_deadline = min(
        deadline, time.monotonic() + max(1.0, deadline - time.monotonic() - 30.0)
    )
